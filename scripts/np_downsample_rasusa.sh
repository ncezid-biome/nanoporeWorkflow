...
#Run rasusa version 0.7.0, latest from staphb dockerhub repo as of 03/07/2023
singularity exec -B $PWD:/data docker://staphb/rasusa:0.7.0 rasusa --version
singularity exec -B $PWD:/data docker://staphb/rasusa:0.7.0 rasusa -g ${GENOMESIZE} -c ${COVERAGE} -i ${READS} -o ${OUTPUT} >> rasusa.log 2>&1